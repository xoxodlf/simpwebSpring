...
<body>
		...
		<form id="boardForm" method="post">
			<table class="table table-inbox table-max">
				...
				<tr class="t_thr">
					...
					<td><input type="hidden" name="group" value="${unknown.group}"/>
						<input type="hidden" name="order" value="${unknown.order}"/>
						<input type="hidden" name="depth" value="${unknown.depth}"/>
						<input type="hidden" name="userNo" value="${unknown.userNo}"/>
						<input type="hidden" name="boardNo" value="${unknown.boardNo}"/></td>
				</tr>
				<tr>
					<td class="tdc">내용 :</td>
			...
			<div class="table-max right" style="width:100%; margin-top: 20px;">
				<a id="modify" data-placement="top" class="tbtn btn"> <i class="ti">답글작성</i></a>
			</div>
		</form>
	</div>
	...
	$("#modify").on('click', (function() {
		var form = $("#boardForm");
		form.attr("action","/ex/unknown/insertComment");
		form.submit();
	}));
	</script>
</body>
